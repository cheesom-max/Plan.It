 ...
 * @param {string} params.firstDayDepartureTime - 첫날 출발 예정 시간 (예: "08:00", 집에서 출발)
 * @param {string} params.lastDayArrivalTime - 마지막날 도착 예정 시간 (예: "20:00", 집에 도착)
 * @param {string} params.transport - 교통수단 (예: "car", "airplane", "train", "bus")
...
export function generateItineraryPrompt({
  ...
  const transportText = transportMap[transport] || transport;
...
- 교통수단: ${transportText}
- 첫날: ${departure}에서 ${firstDayDepartureTime}에 출발 (${transportText}로 이동 후 여행지 도착)
- 마지막날: ${lastDayArrivalTime}까지 ${departure}에 도착해야 함
- 동행: ${companionText}
- 스타일: ${styleTexts}
...
[시간 최적화 - 매우 중요]
- 교통수단: ${transportText}
- 첫날: ${firstDayDepartureTime}에 ${departure}에서 출발. 교통수단별 이동 시간을 고려하여 여행지 도착 후부터 일정 시작
- 마지막날: ${lastDayArrivalTime}까지 ${departure}에 도착해야 하므로, 이동 시간을 역산하여 여행지에서 출발해야 하는 시간 계산

[교통수단별 이동 시간 참고]
- 비행기: 출발 1시간 전 공항 도착 필요, 비행시간 + 공항→시내 이동 시간 고려
- 기차/KTX: 출발 30분 전 역 도착, 역→시내 이동 시간 고려
- 자동차: 출발지에서 목적지까지 직접 이동, 휴게소 휴식 시간 포함
- 버스: 출발 20분 전 터미널 도착, 터미널→시내 이동 시간 고려

예시 (${transportText} 이용 시):
- 서울→제주 비행기: 공항 1시간 전 + 비행 1시간 + 공항→시내 30분 = 약 2.5시간
- 서울→부산 KTX: 역 30분 전 + KTX 2.5시간 = 약 3시간
- 서울→강릉 자동차: 약 2.5시간 (휴게소 포함)
- 서울→전주 버스: 터미널 20분 전 + 버스 3시간 = 약 3.5시간
...
[JSON 형식]
{
  ...
  "budget": {
    "currency": "KRW",
    "accommodation_per_night": 80000,
    "food_per_day": 50000,
    "entrance_fees_total": 30000,
    "total_estimated": 500000,
    "breakdown": "숙박 ${tripDays-1}박, 식비 ${tripDays}일, 입장료 포함"
  },
  "days": [
    {
      "day": 1,
      ...
      "schedule": [
        {
          ...
          "place": {
            ...
            "lat": 37.5665,
            "lng": 126.9780,
            "image_keyword": "장소를 검색할 영문 키워드 (예: jeju beach sunset)"
          },
